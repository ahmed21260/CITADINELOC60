...
export const signInWithGoogle = async () => {
    ...
    try {
        ...
        // On success, the page will redirect. This function will not resolve with a user.
        // The result is handled by getRedirectResult on the destination page.
        return { user: null, error: null };
    } catch (error: any) {
        // This will catch immediate errors, e.g., if the browser blocks redirects.
...
export const createPaymentIntent = async (amount: number, user: User): Promise<{ clientSecret: string; paymentIntentId: string; customerId: string; } | null> => {
    try {
        ...
        const data = result.data as { clientSecret: string; paymentIntentId: string; customerId: string; };

        if (data && data.clientSecret && data.paymentIntentId && data.customerId) {
            return data;
        } else {
            console.error("Error: createPaymentIntent function returned invalid data.", result.data);
            return null;
        }
    } catch (error) {
